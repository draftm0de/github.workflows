...
The action has no explicit inputs. It automatically reads environment variables provided by GitHub Actions:
...
| Name                    | Description |
|-------------------------|-------------|
| `is-pull-request`       | `'true'` when the workflow is running on a `pull_request` event (detected via `GITHUB_HEAD_REF`), `'false'` for push events. |
| `ci-open-pull-request`  | `'true'` when an open PR exists for the current branch during push events, `'false'` when no PR exists. Always `'true'` for PR events. |
...
steps:
  ...
  - name: Run tests only when necessary
    if: ${{ steps.git_state.outputs.is-pull-request == 'true' ||
            steps.git_state.outputs['ci-open-pull-request'] == 'false' }}
    run: npm test
```
...
steps:
  ...
  - name: Deploy to staging
    if: ${{ steps.git_state.outputs.target-branch-name == 'main' }}
...
3. The action sets:
   ...
   - `ci-open-pull-request='true'` (always true for PR events)
   - `source-branch-name=$GITHUB_HEAD_REF`
   - `target-branch-name=$GITHUB_BASE_REF`
4. The action exits immediately (no API calls needed)
...
4. The action queries the GitHub API to count open PRs for the current branch:
   ...
   - If count is `0`: sets `ci-open-pull-request='false'`
   - If count is `> 0`: sets `ci-open-pull-request='true'`
...
```
- [x] is-pull-request
- [x] ci-open-pull-request (current event, is pull_request)
- [x] source-branch-name: feature/new-feature
- [x] target-branch-name: main
```

**For push events (with open PR):**
```
- [ ] is-pull-request
- [x] ci-open-pull-request (PR for the same branch detected)
- [x] source-branch-name: feature/new-feature
- [x] target-branch-name: -
```

**For push events (without open PR):**
```
- [ ] is-pull-request
- [ ] ci-open-pull-request (no PR for the same branch detected)
...
5. **Workflow optimization**: Make intelligent decisions about which jobs to run based on the Git state, reducing CI costs and execution time.
