...
- `tag-levels: 'latest,sha,edge'` + `git-tag-levels: ''` ✅ (validation skipped when git-tag-levels empty)
- `tag-levels: 'patch'` + `git-tag-levels: 'patch'` ✅

**Why**: Ensures git repository tracks at least the same version granularity as Docker registry, preventing version drift between git tags and Docker images.

**Note**:
- Custom tags (`sha`, `edge`, `beta`, `latest`) are ignored in validation
- Validation applies to all version source types (nodejs, flutter, branch)
- To skip validation, leave `git-tag-levels` empty
